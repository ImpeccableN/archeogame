...
#	If the right one is dropped it can't be changed anymore.
		if  $".".texture == null:
			return true
...
func drop_data(_pos, data):
	#item is droppable here
	texture = data["origin_texture"]
	$"../../../Click_correct".play()
	if $"../VBoxContainer_pieces1/piece2".texture == $".".texture:
		$"../VBoxContainer_pieces1/piece2".texture = null
